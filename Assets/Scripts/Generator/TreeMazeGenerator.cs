...
public abstract class TreeMazeGenerator : BasicMazeGenerator {
	...
	public override void GenerateMaze ()
	{
		...
		mCellsToVisit.Add (new CellToVisit (UnityEngine.Random.Range (0, RowCount), UnityEngine.Random.Range (0, ColumnCount),Direction.Start));
        int countForKey = 0;
	    int countForDoor = 0;
        while (mCellsToVisit.Count > 0) {
            ...
            if (movesAvailableCount == 0 && !GetMazeCell(ctv.Row, ctv.Column).IsVisited)
            {
                if (countForKey == 4)
                    GetMazeCell(ctv.Row, ctv.Column).IsKey = true;
                countForKey++;
            
		        if (countForDoor == 5)
		            GetMazeCell(ctv.Row, ctv.Column).IsDoor = true;
		        countForKey++;
		    }
